...
public class SolGryn : Boss_Info
{
    ...
    {
        yield return StartCoroutine(Move_Straight(new Vector3(0, 7, 0), new Vector3(0, 0, 0), 7f, declineCurve));
        ...
        for (int i = 0; i < 4; i++)
        {
            if (SolG_Copy[i].TryGetComponent(out SolGryn_Copy SC1))
                SC1.Move_Lerp_Distance(new Vector3(bangmeon[0, i] * 7, bangmeon[1, i] * 2.5f, 0));
        }
        ...
        yield return StartCoroutine(Change_My_Size(My_Scale, My_Scale / 2, 0.5f, OriginCurve));

        spriteColor.Change_C(new Color(1, 1, 1, 1), 1);
        yield return StartCoroutine(Change_My_Size(My_Scale, My_Scale * 20, 1f, inclineCurve));
    ...
    IEnumerator Boss_Pattern()
    {
        ...
        yield return StartCoroutine(Move_Curve(My_Position, new Vector3(-4, 4, 0),
        ...
        //    My_Position = new Vector3(7, 4, 0);
        //    yield return StartCoroutine(My_Position_Lerp(My_Position, new Vector3(7, 0, 0), 1f, inclineCurve));
    ...
    IEnumerator Pattern01() // 완료
    {
        ...
        yield return StartCoroutine(Change_My_Color_Lerp(SpriteRenderer_Color, new Color(1, 1, 1, 1), 0.33f, 0, DisAppear_Effect_1));

        for (int i = 0; i < 3; i++)
            yield return StartCoroutine(Move_Straight(My_Position, new Vector3(u1[i, 0], u1[i, 1], 0), 0.15f, declineCurve));

        My_Position = new Vector3(4, 2, 0);
        Flash(Color.white, 0, 0.5f);
        yield return StartCoroutine(Change_My_Color_Lerp(new Color(1, 1, 1, 0), new Color(1, 1, 1, 1), 0.5f, 0, DisAppear_Effect_1));

        for (int i = 0; i < 3; i++)
            yield return StartCoroutine(Move_Straight(My_Position, new Vector3(u2[i, 0], u2[i, 1], 0), 0.15f, declineCurve));

        yield return StartCoroutine(Change_My_Color_Lerp(SpriteRenderer_Color, new Color(1, 1, 1, 0), 0.4f, 0, DisAppear_Effect_1));
    ...
    IEnumerator Pattern02()
    {
        ...
        yield return StartCoroutine(Change_My_Color_Lerp(SpriteRenderer_Color, new Color(1, 1, 1, 1), 1f, 0, DisAppear_Effect_1));
        yield return StartCoroutine(Move_Straight(My_Position, new Vector3(My_Position.x, My_Position.y - 1.35f, 0), 0.5f, inclineCurve));
        ...
        if (SolG_Copy[0].TryGetComponent(out SolGryn_Copy SC1) && SolG_Copy[1].TryGetComponent(out SolGryn_Copy SC2))
        {
            ...
            yield return StartCoroutine(Move_Straight(My_Position, new Vector3(My_Position.x, 2.68f, 0), 0.75f, declineCurve));
        ...
        Quaternion Peanut_Lot, Vector3 Peanut_Dir, float Straw_Lot_time_persist, int Peanut_Launch_Num, float Peanut_Launch_Interval)
    {
        ...
        IEnumerator change_color = Change_My_Color_Lerp(new Color(1, 1, 1, 1), new Color(1, 1, 1, 0), Peanut_Launch_Num * Peanut_Launch_Interval + Straw_Lot_time_persist, 0, DisAppear_Effect_1);
    ...
    IEnumerator Pattern03()
    {
        ...
        yield return StartCoroutine(Change_My_Color_Lerp(SpriteRenderer_Color, new Color(1, 1, 1, 1), 0.33f, 0, DisAppear_Effect_1));
        yield return StartCoroutine(Move_Straight(My_Position, new Vector3(My_Position.x, My_Position.y - 3, 0), 1, inclineCurve));
        yield return StartCoroutine(Move_Straight(My_Position, new Vector3(My_Position.x, 11, 0), 1, declineCurve));
        ...
        yield return StartCoroutine(Move_Straight(My_Position, new Vector3(My_Position.x, My_Position.y - 11, 0), 1, declineCurve));
    }
    IEnumerator Pattern04()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            yield return Change_My_Color_Lerp(new Color(1, 1, 1, 0), new Color(1, 1, 1, 0), 0.33f, 1.5f, DisAppear_Effect_2);
        ...
        yield return StartCoroutine(Change_My_Color_Lerp(SpriteRenderer_Color, new Color(1, 1, 1, 1), 1, 0, DisAppear_Effect_1));
        yield return StartCoroutine(Move_Straight(My_Position, new Vector3(7, 0, 0), 1, declineCurve));
    }
    ...
    IEnumerator Pattern05()
    {
        for (int i = 11; i < 14; i++)
        {
            ...
            if (i == 13)
            {
                ...
                Camera_Shake(0.025f, 3, true, false);
            }
            else
                Camera_Shake(0.02f, 1, true, false);
            yield return YieldInstructionCache.WaitForSeconds(1f);
            Destroy(W1);
        }
    ...
    IEnumerator I_OnDie()
    {
        yield return StartCoroutine(Move_Straight(new Vector3(7, 5, 0), new Vector3(7, 0, 0), 3, OriginCurve));
        yield return null;
    }
